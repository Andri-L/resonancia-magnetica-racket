...
- Hora: 1:00AM
- Versión de su código fuente: 1.2.69 
...
- Descripción del programa: Este programa genera una resonancia magnética de 100x100x100 hecha en vectores para buscar puntos y lineas sospechosos en un plano específico. Al final genera un informe de todo lo encontrado.
...
; FUNCIÓN PARA CREAR EL VECTOR CON VALORES ALEATORIOS
( define ( makeVector count #:vtr (vtr #()) )
    ( if ( zero? count )
        vtr
        ( makeVector ( - count 1 ) #:vtr ( vector-append vtr ( vector ( integer->char ( random 21 41 ) ) ) ) )
    ) ; end if
) ; end define makeVector

; FUNCIÓN PARA CREAR EL VECTOR CON VALORES ALEATORIOS DE 10000 EN 10000
( define ( makeVectorByLayer count #:vtr ( vtr #() ) ) 
    ...
    ( if ( zero? count )
        vtr
        ( makeVectorByLayer ( - count 1 ) #:vtr ( vector-append vtr ( makeVector 10000 ) ) )
    ) ; end if zero?
) ; end define makeVectorByLayer

; FUNCIÓN PARA LA CONVERSIÓN DE EL PUNTO EN EL VECTOR A COORDENADAS
...
( define magneticResonance ( makeVectorByLayer 100 ) )
